        ...
        <aside className="w-64 border-r border-zinc-800 bg-zinc-900/40 p-3 space-y-5 overflow-auto shrink-0">
          <div>
            ...
            <div className="space-y-0.5">
              {['All','Downloading','Seeding','Paused','Error','Checking','Queued'].map((s) => {
                ...
                return (
                  <button key={s} onClick={() => setFilter(s)} className={`w-full text-left px-2.5 py-1.5 rounded-lg text-sm flex justify-between items-center ${filter === s ? 'bg-zinc-800 text-white' : 'text-zinc-400 hover:text-white hover:bg-zinc-800/50'}`}>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              {trackerFilter && <button onClick={() => setTrackerFilter(null)} className="text-[11px] text-zinc-500 hover:text-white">clear</button>}
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              {labelFilter && <button onClick={() => setLabelFilter(null)} className="text-[11px] text-zinc-500 hover:text-white">clear</button>}
            </div>
            <div className="space-y-0.5">
              {labels.map(([label, count]) => (
                <button key={label} onClick={() => setLabelFilter(labelFilter === label ? null : label)} className={`w-full text-left px-2.5 py-1.5 rounded-lg text-sm flex justify-between ${labelFilter === label ? 'bg-amber-950 text-amber-200 border border-amber-900' : 'text-zinc-400 hover:text-white hover:bg-zinc-800/50'}`}>
