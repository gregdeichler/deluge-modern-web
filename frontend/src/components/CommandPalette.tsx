...
export default function CommandPalette({ open, onClose, torrents, actions }: { open: boolean; onClose: ()=>void; torrents: any[]; actions: Action[] }) {
  ...
  useEffect(()=>{
    const handler = (e: KeyboardEvent) => { if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase()==='k') { e.preventDefault(); if(open) onClose(); else actions.find(a=>a.id==='palette')?.run() } if(e.key==='Escape' && open) onClose() }
  ...
  const filtered = useMemo(()=>{
    const all = [
      ...actions,
      ...torrents.map(t=>({ id: `torrent-${t.hash}`, label: t.name, section: 'Torrents', run: ()=>{ /* handled by parent */ }, hash: t.hash } as any))
    ]
    if(!q) return all.slice(0,30)
    const low = q.toLowerCase()
    return all.filter(a=>a.label.toLowerCase().includes(low)).slice(0,30)
  }, [q, torrents, actions])

  if (!open) return null
  return (
    <div className="fixed inset-0 z-[60] flex items-start justify-center pt-[20vh] bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-[640px] rounded-2xl border border-zinc-800 bg-zinc-900 shadow-2xl overflow-hidden" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center gap-3 px-4 h-14 border-b border-zinc-800">
          ...
          <span className="text-[11px] text-zinc-600">ESC to close</span>
        </div>
        <div className="max-h-[380px] overflow-auto p-2">
          {['Actions','Torrents'].map(section=>(
            <div key={section} className="mb-2">
              ...
              {filtered.filter(f=>f.section===section).map((a:any)=>(
                <button key={a.id} onClick={()=>{ onClose(); a.run() }} className="w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-800 text-sm flex justify-between">
                  <span className="truncate">{a.label}</span>
                  {a.hotkey && <span className="text-xs text-zinc-500">{a.hotkey}</span>}
                </button>
              ))}
            </div>
          ))}
