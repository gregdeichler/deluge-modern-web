...
function FilesTab({ hash }: { hash: string }) {
  ...
  if (error) {
    return <div className="p-4 text-xs text-red-400">{(error as any).message}</div>
  }
  if (!data) {
  ...
  return (
    <div className="p-2">
      ...
      {peers.map((p: any, i: number) => (
        <div key={i} className="grid grid-cols-[1fr_80px_80px_100px] text-xs px-2 py-1.5 border-b border-zinc-900/50 hover:bg-zinc-800/20">
    ...
    <div className="p-3 text-xs">
      <div className="space-y-1">
        {trackers.map((tr: any, i: number) => (
          <div key={i} className="flex justify-between items-center border border-zinc-800 rounded-lg px-3 py-2 bg-zinc-900/50">
            <span className="truncate pr-4">{tr.url}</span>
            <span className="text-[11px] px-2 py-0.5 rounded-full bg-zinc-800">{tr.tier}</span>
          </div>
        ))}
    ...
    <div className="h-[340px] border-t border-zinc-800 bg-zinc-900 flex flex-col shrink-0">
      <div className="h-11 flex items-center justify-between px-3 border-b border-zinc-800 shrink-0">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs truncate max-w-[420px] hidden md:block text-zinc-300" title={status?.name}>{status?.name}</span>
          <button onClick={onClose} className="w-7 h-7 rounded-full bg-zinc-800 hover:bg-zinc-700 flex items-center justify-center">✕</button>
        </div>
      </div>
      <div className="flex-1 overflow-auto bg-zinc-950">
        ...
        {tab === 'options' && (
          <div className="p-4 text-xs text-zinc-400 space-y-2">
            <div>Save path: <span className="text-white">{status?.save_path}</span></div>
